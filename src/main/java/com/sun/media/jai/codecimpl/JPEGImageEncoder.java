...
public class JPEGImageEncoder extends ImageEncoderImpl {
  ...
  //
  static void modifyEncodeParam(
      final JPEGEncodeParam jaiEP,
      final com.sun.image.codec.jpeg.JPEGEncodeParam j2dEP,
      final int nbands) {
  ...
  public void encode(final RenderedImage im) throws IOException {
    ...
    if (((transType != DataBuffer.TYPE_BYTE) && !CodecUtils.isPackedByteImage(im))
    ...
    if (im instanceof BufferedImage) {
      ...
      if (im.getNumXTiles() == 1 && im.getNumYTiles() == 1) {
        ...
        final WritableRaster target = sampleModel.getSampleSize(0) == 8
      ...
      if (ras instanceof WritableRaster) {
        ...
        wRas = Raster.createWritableRaster(
            ras.getSampleModel(),
            ras.getDataBuffer(),
            new Point(ras.getSampleModelTranslateX(), ras.getSampleModelTranslateY()));
      }
    ...
    if (colorModel instanceof IndexColorModel) {
      ...
      if (bi.getSampleModel().getNumBands() == 4) {
        ...
        //
        final WritableRaster rgbaRas = bi.getRaster();
        final WritableRaster rgbRas = rgbaRas
            .createWritableChild(0, 0, bi.getWidth(), bi.getHeight(), 0, 0, new int[]{ 0, 1, 2 });
        //
        ...
        //
        final PackedColorModel pcm = (PackedColorModel) bi.getColorModel();
        final int bits = pcm.getComponentSize(0) + pcm.getComponentSize(1) + pcm.getComponentSize(2);
        final DirectColorModel dcm = new DirectColorModel(bits, pcm.getMask(0), pcm.getMask(1), pcm.getMask(2));
        bi = new BufferedImage(dcm, rgbRas, false, null);
      }
    }

    // Create the Java2D encodeParam based on the BufferedImage
    final com.sun.image.codec.jpeg.JPEGEncodeParam j2dEP = com.sun.image.codec.jpeg.JPEGCodec
    ...
    encoder = com.sun.image.codec.jpeg.JPEGCodec.createJPEGEncoder(this.output, j2dEP);

    try {
      // Write the image data.
      encoder.encode(bi);
    } catch (final IOException e) {
      final String message = JaiI18N.getString("JPEGImageEncoder2");
