...
public class TIFFImageEncoder extends ImageEncoderImpl {
  ...
  private int encode(RenderedImage im, final TIFFEncodeParam encodeParam, final int ifdOffset, final boolean isLast)
      throws IOException {
    // Cannot store a packed byte image directly so reformat it.
    if (CodecUtils.isPackedByteImage(im)) {
      // Get the source ColorModel.
      final ColorModel sourceCM = im.getColorModel();

      // Create an equivalent ComponentColorModel.
      final ColorModel destCM = RasterFactory.createComponentColorModel(
      ...
      final Point origin = new Point(im.getMinX(), im.getMinY());
      final WritableRaster raster = Raster
          .createWritableRaster(destCM.createCompatibleSampleModel(im.getWidth(), im.getHeight()), origin);
    ...
    if (compression == COMP_JPEG_TTN2) {
      if (imageType == TIFF_PALETTE) {
        throw new RuntimeException(JaiI18N.getString("TIFFImageEncoder11"));
          ...
          && (imageType == TIFF_GRAY || imageType == TIFF_RGB || imageType == TIFF_YCBCR))) {
        throw new RuntimeException(JaiI18N.getString("TIFFImageEncoder9"));
      }
    }

    // Check bilevel encoding compatibility.
    if ((imageType != TIFF_BILEVEL_WHITE_IS_ZERO && imageType != TIFF_BILEVEL_BLACK_IS_ZERO)
        && (compression == COMP_GROUP3_1D || compression == COMP_GROUP3_2D || compression == COMP_GROUP4)) {
    ...
    if (compression == COMP_JPEG_TTN2) {
      ...
      for (int i = 1; i < numBands; i++) {
        final int subH = jep.getHorizontalSubsampling(i);
        if (subH > maxSubH) {
          maxSubH = subH;
        }
        final int subV = jep.getVerticalSubsampling(i);
        if (subV > maxSubV) {
          maxSubV = subV;
        }
      }

      final int factorV = 8 * maxSubV;
    ...
    final long bytesPerRow = (long) Math.ceil((sampleSize[0] / 8.0) * tileWidth * numBands);
    ...
    final long totalBytesOfData = bytesPerTile * (numTiles - 1) + tileByteCounts[numTiles - 1];
    ...
    fields.add(
        new TIFFField(TIFFImageDecoder.TIFF_BITS_PER_SAMPLE, TIFFField.TIFF_SHORT, numBands, intsToChars(sampleSize)));

    fields.add(
        new TIFFField(TIFFImageDecoder.TIFF_COMPRESSION, TIFFField.TIFF_SHORT, 1, new char[]{ (char) compression }));
    ...
    fields.add(
        new TIFFField(TIFFImageDecoder.TIFF_SAMPLES_PER_PIXEL, TIFFField.TIFF_SHORT, 1, new char[]{ (char) numBands }));
    ...
    if (colormap != null) {
      fields.add(
          new TIFFField(TIFFImageDecoder.TIFF_COLORMAP, TIFFField.TIFF_SHORT, sizeOfColormap, intsToChars(colormap)));
    }
    ...
    if (numExtraSamples > 0) {
      final int[] extraSamples = new int[numExtraSamples];
    ...
    if (dataType != DataBuffer.TYPE_BYTE) {
      ...
      fields.add(
          new TIFFField(
              TIFFImageDecoder.TIFF_SAMPLE_FORMAT,
              TIFFField.TIFF_SHORT,
              numBands,
    ...
    final boolean inverseFill = encodeParam.getReverseFillOrder();
    final boolean T4encode2D = encodeParam.getT4Encode2D();
    final boolean T4PadEOLs = encodeParam.getT4PadEOLs();
    TIFFFaxEncoder faxEncoder = null;

    // Add bilevel compression fields.
    if ((imageType == TIFF_BILEVEL_BLACK_IS_ZERO || imageType == TIFF_BILEVEL_WHITE_IS_ZERO)
        && (compression == COMP_GROUP3_1D || compression == COMP_GROUP3_2D || compression == COMP_GROUP4)) {
      ...
      fields.add(
          new TIFFField(
              TIFFImageDecoder.TIFF_FILL_ORDER,
              TIFFField.TIFF_SHORT,
              1,
      ...
      if (compression == COMP_GROUP3_2D) {
        ...
        // T6Options field.
        fields.add(new TIFFField(TIFFImageDecoder.TIFF_T6_OPTIONS, TIFFField.TIFF_LONG, 1, new long[]{ 0x00000000 }));
      }
    }
...
//    }
    ...
    if (imageType == TIFF_YCBCR) {
      ...
      fields.add(
          new TIFFField(
              ...
              TIFFField.TIFF_SHORT,
              2,
              new char[]{ (char) subsampleH, (char) subsampleV }));
      ...
      long[][] refbw;
      if (compression == COMP_JPEG_TTN2) {
        refbw = new long[][]{ // no headroon/footroom
            { 0, 1 },
            { 255, 1 },
            { 128, 1 },
            { 255, 1 },
    ...
    if (extraFields != null) {
      final ArrayList extantTags = new ArrayList(fields.size());
    ...
    final int deflateLevel = Deflater.DEFAULT_COMPRESSION;
    ...
    final boolean checkContiguous = ((sampleSize[0] == 1
        ...
        && dataType == DataBuffer.TYPE_BYTE) || (sampleSize[0] == 8 && sampleModel instanceof ComponentSampleModel));
    ...
    for (int row = minY; row < lastRow; row += tileHeight) {
      final int rows = isTiled ? tileHeight : Math.min(tileHeight, lastRow - row);
      final int size = rows * tileWidth * numBands;

      for (int col = minX; col < lastCol; col += tileWidth) {
        // Grab the pixels
        final Raster src = im.getData(new Rectangle(col, row, tileWidth, rows));

        boolean useDataBuffer = false;
        if (compression != COMP_JPEG_TTN2) { // JPEG access Raster
          if (checkContiguous) {
            if (sampleSize[0] == 8) { // 8-bit
              final ComponentSampleModel csm = (ComponentSampleModel) src.getSampleModel();
              ...
              final MultiPixelPackedSampleModel mpp = (MultiPixelPackedSampleModel) src.getSampleModel();
        ...
        switch (sampleSize[0]) {
          ...
          case 1:
            ...
            if (useDataBuffer) {
              final byte[] btmp = ((DataBufferByte) src.getDataBuffer()).getData();
              final MultiPixelPackedSampleModel mpp = (MultiPixelPackedSampleModel) src.getSampleModel();
            ...
            if (compression == COMP_NONE) {
              ...
              final int rowStride = (tileWidth + 7) / 8;
              int rowOffset = 0;
              int numCompressedBytes = 0;
              for (int tileRow = 0; tileRow < rows; tileRow++) {
                final int numCompressedBytesInRow = faxEncoder.encodeRLE(bpixels, rowOffset, 0, tileWidth, compressBuf);
              ...
              final int numCompressedBytes = faxEncoder.encodeT4(
                  !T4encode2D, // 1D == !2D
                  T4PadEOLs,
                  bpixels,
                  (tileWidth + 7) / 8,
              ...
              final int numCompressedBytes = faxEncoder
                  .encodeT6(bpixels, (tileWidth + 7) / 8, 0, tileWidth, rows, compressBuf);
              tileByteCounts[tileNum++] = numCompressedBytes;
              this.output.write(compressBuf, 0, numCompressedBytes);
          ...
          case 8:
            ...
            if (compression != COMP_JPEG_TTN2) {
              if (useDataBuffer) {
                ...
                final ComponentSampleModel csm = (ComponentSampleModel) src.getSampleModel();
                int inOffset = csm
                    .getOffset(col - src.getSampleModelTranslateX(), row - src.getSampleModelTranslateY());
                final int lineStride = csm.getScanlineStride();
                if (lineStride == (int) bytesPerRow) {
                  System.arraycopy(btmp, inOffset, bpixels, 0, (int) bytesPerRow * rows);
                } else {
...
//              tileByteCounts[tileNum++] = (int) (endPos - startPos);
  ...
  private void writeDirectory(final int thisIFDOffset, final SortedSet fields, final int nextIFDOffset)
      throws IOException {
    ...
    final ArrayList tooBig = new ArrayList();
  ...
  private void writeValuesAsFourBytes(final TIFFField field) throws IOException {
    ...
    switch (dataType) {
      ...
      case TIFFField.TIFF_SHORT:
        final char shorts[] = field.getAsChars();
      ...
      case TIFFField.TIFF_SSHORT:
        final short sshorts[] = field.getAsShorts();
  ...
  private void writeValues(final TIFFField field) throws IOException {
    ...
    switch (dataType) {
      ...
      // signed 32 bits
      case TIFFField.TIFF_SLONG:
        final int slongs[] = field.getAsInts();
        for (int i = 0; i < count; i++) {
          writeLong(slongs[i]);
        }
        break;
      ...
      case TIFFField.TIFF_SRATIONAL:
        final int srationals[][] = field.getAsSRationals();
        for (int i = 0; i < count; i++) {
   ...
   */
  private static int compressPackBits(
      final byte[] data,
      final int numRows,
   ...
   */
  private static int packBits(final byte[] input, int inOffset, final int inCount, final byte[] output, int outOffset) {
  ...
  private static int deflate(final Deflater deflater, final byte[] inflated, final byte[] deflated) {
