...
public class ColorModelState extends SerializableStateImpl {
   ...
   */
  private static int[] getPredefinedColorSpace(final ColorSpace cs) {
    // Initialize an array of the pre-defined ColorSpaces.
    final int[] colorSpaces = new int[]{
   ...
   */
  private static boolean serializeColorSpace(final ColorSpace cs, final ObjectOutputStream out) throws IOException {
    final int[] colorSpaceType = getPredefinedColorSpace(cs);
    final boolean isICCColorSpace = (cs instanceof ICC_ColorSpace);
    ...
    if (colorSpaceType == null) {
      ...
      boolean flag = false;
      try {
        final Class cls = cs.getClass();
        final Method getInstance = cls.getMethod("getInstance", null);
   ...
   */
  private static ColorSpace deserializeColorSpace(final ObjectInputStream in)
      throws IOException,
      ClassNotFoundException {
    ColorSpace cs = null;
    final int colorSpaceType = in.readInt();
  ...
  /**
    ...
    */
  public ColorModelState(final Class c, final Object o, final RenderingHints h) {
    super(c, o, h);
  }
  ...
  private void writeObject(final ObjectOutputStream out) throws IOException {
    final ColorModel colorModel = (ColorModel) this.theObject;

    // Write serialized form to the stream.
    if (colorModel == null) {
      out.writeInt(COLORMODEL_NULL);
    } else if (colorModel instanceof ComponentColorModel) {
      final ComponentColorModel cm = (ComponentColorModel) colorModel;
      ...
      final IndexColorModel cm = (IndexColorModel) colorModel;
      out.writeInt(COLORMODEL_INDEX);
      final int size = cm.getMapSize();
      final int[] cmap = new int[size];
      cm.getRGBs(cmap);
      ...
      final boolean csSerialized = serializeColorSpace(cm.getColorSpace(), out);
      if (!csSerialized) {
        out.writeBoolean(cm.hasAlpha());
      }
      out.writeInt(cm.getPixelSize());
